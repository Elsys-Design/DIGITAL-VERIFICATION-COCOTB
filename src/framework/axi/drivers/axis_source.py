    ...
    async def write_data(self, data: Data) -> None:
        ...
        if not self.has_tkeep and data.length % self.bus_size != 0:
            self.logger.warning(
                ...
                "added to the end of the transaction".format(
                    data.length, self.bus_size, data.length % self.bus_size
                )
            )
        ...
        self.logger.info(
            "Writting Data(tdest={}, length={})".format(data.addr, data.length)
        )

        # tkeep is supported by default by cocotbext.axi.AxiStreamSource
        await self.write_frame(
            cocotbext.axi.AxiStreamFrame(tdata=data.data, tdest=data.addr)
        )
