...
class TB:
    ...
    def __init__(self, dut):
        ...
        self.monitor_in = AxiMonitor("in_axi", self.bus_in, dut.aclk, dut.aresetn, reset_active_level=False)
        ...
        for name in names:
            ...
            self.out_axi_bus.append(
                    AxiBus.from_prefix(dut, name)
            )
            # Building the cocotb axilite ram
            self.out_axi_rams.append(
                    AxiRam(self.out_axi_bus[-1], dut.aclk, dut.aresetn, reset_active_level=False, size=2**16)
            )
            # Building the monitor
            self.out_axi_monitors.append(
                    AxiMonitor(name, self.out_axi_bus[-1], dut.aclk, dut.aresetn, reset_active_level=False)
            )
