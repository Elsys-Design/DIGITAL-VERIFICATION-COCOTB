...
class TB:
    ...
    def __init__(self, dut):
        ...
        self.out_monitors = []
        out_names = [
                (["out0_axilite", "out1_axilite"], [AxiLiteBus, AxiLiteRam, AxiLiteMonitor]),
        ...
        for names, types in out_names:
            for name in names:
                ...
                self.out_rams.append(
                        types[1](self.out_bus[-1], dut.aclk, dut.aresetn, reset_active_level=False, size=2**16)
                )
                # Building the monitor
                self.out_monitors.append(
                        types[2](name, self.out_bus[-1], dut.aclk, dut.aresetn, reset_active_level=False)
                )
